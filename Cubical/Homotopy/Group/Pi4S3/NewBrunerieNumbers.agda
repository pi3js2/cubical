...
open import Cubical.Foundations.Prelude using
  (_,_ ; fst ; snd ; _≡_ ; cong ; _∙_ ; refl ; isProp→isSet ; J ; sym ; isProp ; isSet ; isGroupoid ; is2Groupoid ; toPathP ; funExt ; transportRefl ; Square)
...
open import Cubical.Foundations.Transport using
  (transportRefl)
...
myFun* : (x : S²') (p q : Path ∥ S²' ∥₂ ∣ base ∣₂ ∣ x ∣₂) → Type
myFun* base p q = Square p p q q
myFun* (surf i i₁) p q = {!Square p p q q!}
...
myFun : (p q : Path ∥ S²' ∥₂ ∣ base ∣₂ ∣ base ∣₂)
  → Square p p q q
  → (i : I)
  → codeTruncS²' (q i) .fst -- Ω ∥ S¹∙ ∥₁∙ .fst
myFun p q t i = coe0→1 (λ j → codeTruncS²' (t i j) .fst) {!!}



g8* : {p : Path ∥ S²' ∥₂ ∣ base ∣₂ ∣ base ∣₂}
  → Square p p (sym p) (sym p) → Ω ∥ S¹∙ ∥₁∙ .fst 
g8* = {!!}
...
g9* : {x : ∥ S¹ ∥₁} → x ≡ x → ∥ ℤ ∥₀
g9* {x = x} = elim₁ {B = λ x → x ≡ x → ∥ ℤ ∥₀}
  (λ _ → isOfHLevelΠ 3 λ _ → Cubical.Foundations.HLevels.isSet→isGroupoid squash₀)
  (λ { base → F
    ; (loop i) → F-c i})
  x
  where
  ...
  F : ∣ base ∣₁ ≡ ∣ base ∣₁ → ∥ ℤ ∥₀
  F p = coe0→1 (λ i → codeTruncS¹ (p i) .fst) ∣ pos 0 ∣₀

  F-c : PathP (λ i → ∣ loop i ∣₁ ≡ ∣ loop i ∣₁ → ∥ ℤ ∥₀) F F
  F-c = toPathP (funExt λ p → transportRefl (F (coe0→1 (λ i → ∣ loop (~ i) ∣₁ ≡ ∣ loop (~ i) ∣₁) p))
              ∙ cong F {!!})
...
η₂' (inl x) = inr base
η₂' (inr x) = inr base
η₂' (push base base i) = inr base
η₂' (push base (loop i₁) i) = inr base
η₂' (push (loop i₁) base i) = inr base
η₂' (push (loop a) (loop b) i) =
  hcomp (λ k → λ {(a = i0) → {!sym (push (loop b) base) ∙ push (loop b) (loop b)!}
                 ; (a = i1) → {!sym (push (loop b) base) ∙ push (loop b) (loop b)!}
                 ; (b = i0) → {!sym (push (loop (~ a)) (loop (~ a)) ) ∙ push (loop (~ a)) base!}
                 ; (b = i1) → {!sym (push (loop (~ a)) (loop (~ a)) ) ∙ push (loop (~ a)) base!}
                 ; (i = i0) → {!!}
                 ; (i = i1) → {!!}})
        ((sym (push (loop b · invLooper (loop a)) (invLooper (loop a)))
        ∙ push (loop b · invLooper (loop a)) (loop b)) i)
-}
...
_→∙_ : Pointed₀ → Pointed₀ → Type
A →∙ B = Σ (fst A → fst B) λ f → f (snd A) ≡ snd B

Ω→ : {A B : Pointed₀} → A →∙ B → Ω A →∙ Ω B
fst (Ω→ f) p = sym (snd f) ∙∙ cong (fst f) p ∙∙ snd f
snd (Ω→ f) = ∙∙lCancel (snd f)
...
η₂ (inr x) = inr x
η₂ (push a b i) =
    (sym (push (b · invLooper a) (invLooper a)) ∙∙ refl ∙∙ push (b · invLooper a) b) i


β₂ : ℤ
β₂ = g10 (g9 (g8* {p = λ i → f7 λ k → joinS¹S¹→SuspS²' (η₂ (push (loop i) (loop i0) k))} λ i j → f7 λ k → joinS¹S¹→SuspS²' (η₂ (push (loop i) (loop j) k)))) -- {!(p : (f7 λ k → joinS¹S¹→SuspS²' (η₂ (push (loop ?) (loop i0) k))) ≡ (f7 λ k → joinS¹S¹→SuspS²' (η₂ (push (loop ?) (loop i0) k)))) → p ≡ refl!})) -- f7 λ k → {!joinS¹S¹→SuspS²' (η₂ (push (loop i) (loop j) k))!})) -- (λ k → joinS¹S¹→SuspS²' (η₂ (push (loop i) (loop j) k)))))
