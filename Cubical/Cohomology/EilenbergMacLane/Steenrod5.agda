...
{-
  asPP'→ΠR-extend : asPushout' → ΠR-extend
  asPP'→ΠR-extend (inl x) = ΠR-extend**↑→ΠR-extend** x
  asPP'→ΠR-extend (inr x) = inr (inl ((inr (fst x)) , (snd x)))
  asPP'→ΠR-extend (push (e , ppl f) i) = inr (push ((inr e) , f) (~ i))
  asPP'→ΠR-extend (push (e , ppr i₁ a b) i) =
    push (i₁ , (inl ((inr e) , (a , b)))) i
  asPP'→ΠR-extend (push (e , pplr i₁ f i₂) i) =
    hcomp (λ k → λ {(i = i0) → ΠR-extend**↑→ΠR-extend**
                                  (doubleCompPath-filler
                                    ...
                                    (sym (pashₗ i₁ ((fst e i₁)
                                                 , ((f i₁ (fst e i₁))
                                                 , (f (notI i₁)))))) k i₂)
                   ; (i = i1) → inr {!push (inr e , f) (~ k ∨ ~ i₂)!} {- inr (compPath-filler'
                                      (λ j → inl (inr e , elimIηPushTop i₁ (inr e) f j))
                                      (push (inr e , f)) i₂ (~ k)) -}
                   ; (i₂ = i0) → sqr {A = ΠR-extend} (λ k → inr (push (inl (fst e i₁) , f) k))
                                                      (λ i → inr (push (inr e , f) (~ i))) k i -- inr (push (inr e , f) (~ i ∨ ~ k))
                   ; (i₂ = i1) → {!sqr {A = ΠR-extend} !} -- push (i₁ , push (inr e , f) (~ k)) i
                   })
         {!!}
-}
  ...
  {-
    hcomp (λ k → λ {(i = i0) → ΠR-extend**↑→ΠR-extend**
                   ...
                   ; (i = i1) → inr (compPath-filler'
                                      (λ j → inl (inr e , elimIηPushTop i₁ (inr e) f j))
                                      (push (inr e , f)) i₂ (~ k))
                   ; (i₂ = i0) → inr (push (inr e , f) (~ i ∨ ~ k))
                   ; (i₂ = i1) → push (i₁ , push (inr e , f) (~ k)) i
                   })
      (hcomp (λ k → λ {(i = i0) → ΠR-extend**↑→ΠR-extend**
                                  (doubleCompPath-filler
                                    (λ k → inr (push (fst e i₁ , f) (~ k)))
                                    ...
                                    (sym (pashₗ i₁ ((fst e i₁)
                                                 , ((f i₁ (fst e i₁))
                                                 , (f (notI i₁)))))) k i₂)
                   ; (i = i1) → push (i₁ , inr ((fst e i₁) , f)) k  -- inr (push (inl (fst e i₁) , f) (k ∨ i₂)) -- inr (push {!? , f!} k) -- inr (push (inl (fst e i₁) , f) k)
                   ; (i₂ = i0) → {!push (inl (fst e i₁) , f) k!} -- inr (push (inl (fst e i₁) , f) k) -- inr (push (inl (fst e i₁) , f) k)
                   ; (i₂ = i1) → {!elimIη (λ j → f j (fst e i₁)) (i ∨ ~ k)!} -- cb k i
                   })
            {!inr (push (inl (fst e i₁) , f) (i₂ ∨ ~ i))!}) -- (inr (inl (inl (fst e i₁) , elimIη (λ j → f j (fst e i₁)) i₁ (~ i₂))))) -- (inr (inl (inl (fst e i₁) , elimIη (λ j → f j (fst e i₁)) i₁ (~ i₂ ∨ i)))))
    where
    cb : Square {A = ΠR-extend}
          (λ i → inr (inl (inl (fst e i₁) , elimIη (λ j → f j (fst e i₁)) i₁ i)))
          (push (i₁ , inr (fat→ᵣ i₁ (inr e , f))))
          (λ k → push (i₁ , inl (inl (fst e i₁) , f i₁ (fst e i₁) , f (notI i₁))) (~ k))
          (λ k → inr (push (inl (fst e i₁) , f) k))
    cb k i =
      hcomp (λ r → λ {(i = i0) → push (i₁ , inl (inl (fst e i₁)
                                      , f i₁ (fst e i₁) , f (notI i₁))) (~ k ∧ r)
                   ; (i = i1) → {!!} -- push (i₁ , {!!}) r -- inr (push (inl (fst e i₁) , f) (k ∧ r)) -- inr (push (inl (fst e i₁) , f) k)
                   ; (k = i0) → {!inr (inl (inl (fst e i₁) , elimIη (λ j → f j (fst e i₁)) i₁ (i ∨ ~ r)))!} -- push ({!i₁!} , {!!}) (r ∨ i)
                   {-
                   inr (inl ((inl (fst e i₁))
                                   , (elimIη (λ j → f j (fst e i₁)) i₁ i)))
                                   -}
                   ; (k = i1) → push (i₁ , inr (fat→ᵣ i₁ (inr e , f))) (i ∧ r)})
            (inl {!!})
-}
...
--   PushTopIsoBool : 2nd.PushTop ≃ 1st.PushTop
--   PushTopIsoBool = Σ-cong-equiv-snd λ i
--     → {!FFmain J A!} -- (equivToIso (PushTop≃Main I i J A .fst))

--   Π-extend→Bool : 2nd.ΠR-extend → 1st.ΠR-extend
--   Π-extend→Bool = {!!}
...
ΠR-base**→JoinR : ∀ {ℓ} (I J : RP∞' ℓ)
  (A : fst I → fst J → Type ℓ)
  → 2-elter'.ΠR-base* I (fst J) A
  → joinR-gen (fst J) (λ j → joinR-gen (fst I) (λ i → A i j))
ΠR-base**→JoinR I J A (inl x) = inlR (fst x , inrR (snd x))
ΠR-base**→JoinR I J A (inr x) = inrR λ i → inrR λ j → x j i
ΠR-base**→JoinR I J A (push a i) =
  push* (fst a , inrR (λ j → snd a j (fst a)))
        (λ j → inrR λ i → snd a i j)
        refl i

ΠR-extend**→JoinR : ∀ {ℓ} (I J : RP∞' ℓ)
  (A : fst I → fst J → Type ℓ)
  → 2-elter'.ΠR-extend** I (fst J) A
  → joinR-gen (fst J) (λ j → joinR-gen (fst I) (λ i → A i j))
ΠR-extend**→JoinR I J A =
  2-elter'.ΠR-extend**-elim I (fst J) A
    (λ x → inlR ((snd x .fst .fst) , (inrR (2-elter'.elimI I (fst J) A (fst x) (x .snd .fst .snd) (x .snd .snd (snd x .fst .fst))))))
    (ΠR-base**→JoinR I J A)
    λ _ _ → refl

{-

asPushoutBack→JoinR : ∀ {ℓ} (I : RP∞' ℓ)
  (A : fst I → fst I → Type ℓ)
  (x : 2-elter'.asPushoutBack I (fst I) A (idEquiv (fst I)))
  → ΠR-extend**→JoinR I I A
      (2-elter'.asPushoutBack→ₗ I (fst I) A (idEquiv (fst I) , x))
   ≡ inrR (λ i → inlR (i , 2-elter'.asPushoutBack→ᵣ-pre I (fst I) A (idEquiv (fst I)) x i))
asPushoutBack→JoinR I A (2-elter'.ppl f) j =
  inrR λ i → push* (i , f i i) (λ j → f j i) refl (~ j)
asPushoutBack→JoinR I A (2-elter'.ppr i a b) =
  push* (i , (inrR (2-elter'.elimI I (fst I) A i a (b (fst (idEquiv (fst I)) i)))))
        (λ i₁ →
            inlR
            (i₁ ,
             2-elter'.asPushoutBack→ᵣ-pre I (fst I) A (idEquiv (fst I))
             (2-elter'.ppr i a b) i₁))
        (push* (i , 2-elter'.elimI I (fst I) A {B = λ i → A i i} i a (b (snd I .fst .fst i)) i)
          (2-elter'.elimI I (fst I) A i a (b i))
          (2-elter'.elimIβ I (fst I) A i a (b i) .fst
        ∙ sym (2-elter'.elimIβ I (fst I) A i a (b (snd I .fst .fst i)) .fst)))
asPushoutBack→JoinR I A (2-elter'.pplr i f j) k =
  hcomp (λ r → λ {(k = i0) → {!!} -- ΠR-extend**→JoinR I I A {!!}
                 ; (k = i1) → {!f!}
                 ; (j = i0) → {!!}
                 ; (j = i1) → {!!}})
        {!!}
...
{-
k = i0 ⊢ ΠR-extend**→JoinR I I A
         (2-elter'.pashᵣ i (idfun (fst I) i , f) (~ j))
k = i1 ⊢ inrR
         (λ i₁ →
            inlR
            (i₁ ,
             2-elter'.elimIη I (fst I) A (λ i₂ → f i₂ (idfun (fst I) i₂)) i
             (~ j) i₁))
j = i0 ⊢ inrR
         (λ i₁ →
            push* (i₁ , f i₁ i₁) (λ j₁ → f j₁ i₁) (λ _ → f i₁ i₁) (~ k))
j = i1 ⊢ push*
         (i ,
          inrR
          (2-elter'.elimI I (fst I) A i (f i (fst (idEquiv (fst I)) i))
           (f (2-elter'.notI I (fst I) A i) (fst (idEquiv (fst I)) i))))
         (λ i₁ →
            inlR
            (i₁ ,
             2-elter'.asPushoutBack→ᵣ-pre I (fst I) A (idEquiv (fst I))
             (2-elter'.ppr i (f i (fst (idEquiv (fst I)) i))
              (f (2-elter'.notI I (fst I) A i)))
             i₁))
         (push*
          (i ,
           2-elter'.asPushoutBack→ᵣ-pre I (fst I) A (idEquiv (fst I))
          ...
          (2-elter'.elimI I (fst I) A i (f i (fst (idEquiv (fst I)) i))
           (f (2-elter'.notI I (fst I) A i) (fst (idEquiv (fst I)) i)))
          (2-elter'.elimIβ I (fst I) A i (f i (fst (idEquiv (fst I)) i))
           (f (2-elter'.notI I (fst I) A i) (fst (idEquiv (fst I)) i)) .fst
           ∙
           (λ i₁ →
              2-elter'.elimIβ I (fst I) A i (f i (fst (idEquiv (fst I)) i))
              (f (2-elter'.notI I (fst I) A i)
               (fst (idEquiv (fst I)) (2-elter'.notI I (fst I) A i)))
              .fst (~ i₁))))
         k
-}

asPushout→JoinR : ∀ {ℓ} (I J : RP∞' ℓ)
  (A : fst I → fst J → Type ℓ)
  → 2-elter'.asPushout I (fst J) A
  → joinR-gen (fst J) (λ j → joinR-gen (fst I) (λ i → A i j))
asPushout→JoinR I =
  asPushout-elim' I
    (λ J A _ → joinR-gen (fst J) (λ j → joinR-gen (fst I) (λ i → A i j)))
      (ΠR-extend**→JoinR I)
      (λ A b → inrR λ i → inlR (i , b i))
      (asPushoutBack→JoinR I) -- (asPushoutBack→JoinR I)
