...
module _ (A B : Type) where
  ...
  pick : Bool × (A × B) → A ⊎ B
  pick (true , a) = inl (fst a)
  pick (false , a) = inr (snd a)

  JJ : Type
  JJ = Pushout pick snd

JJJ : (A : Bool → Bool → Type)
  → Type
JJJ A = JJ (JJ (A true true) (A true false)) (JJ (A false true) (A false false))

ASA : isProp (Σ[ F ∈ ((A : Bool → Bool → Type) → JJJ A ≃ JJJ (λ x y → A y x)) ]
             ((A : _) → F A ≡ invEquiv (F (λ x y → A y x))))
fst (ASA (F , p) (F' , p') i) A = {!p A!}
snd (ASA (F , p) (F' , p') i) A = {!!}

barr : ∀ {ℓ} (X Y : RP∞' ℓ) (A : fst X → fst Y → Type ℓ) → Type _
barr X Y A = {!(x : fst X) (y : fst Y) → ?!}

JJJ' :
  Iso (Σ[ F ∈ ((A : Bool → Bool → Type) → JJJ A ≃ JJJ (λ x y → A y x)) ]
             ((A : _) → F A ≡ invEquiv (F (λ x y → A y x))))
      (Σ[ F ∈ ((A : Bool → Bool → Type) → JJJ A ≃ JJJ A) ]
             ((A : _) → F A ≡ idEquiv _))
Iso.fun JJJ' (F , p) = (λ A → compEquiv (F A) (F (λ x y → A y x)))
                     , λ A → Σ≡Prop isPropIsEquiv
                     (funExt λ t → cong (F (λ x y → A y x) .fst) (funExt⁻ (cong fst (p A)) t)
                     ∙ secEq (F (λ x y → A y x)) t)
Iso.inv JJJ' (F , p) = {!!} , {!p!}
Iso.rightInv JJJ' = {!!}
Iso.leftInv JJJ' = {!!}
...
--   module _ (a0 : A) (b0 : B) where
--     PIso : (C : _) → Iso (JJ → C) (Σ[ f ∈ (A → B → C) ] ((a a' : A) (b b' : B) → f a b ≡ f a' b'))
--     Iso.fun (PIso C) f = (λ a b → f (inr(a , b))) , λ a a' b b'
--       → cong f ((sym (push (true , a , b))))
--       ∙ cong f ((push (true , a , b')))
--       ∙ cong f ((sym (push (false , a , b'))))
--       ∙ cong f (push (false , a' , b'))
--     Iso.inv (PIso C) (f , p) (inl (inl x)) = f x b0
--     Iso.inv (PIso C) (f , p) (inl (inr x)) = f a0 x
--     Iso.inv (PIso C) (f , p) (inr (a , b)) = f a b
--     Iso.inv (PIso C) (f , p) (push (false , s) i) = p a0 (fst s) (snd s) (snd s) i
--     Iso.inv (PIso C) (f , p) (push (true , s) i) = p (fst s) (fst s) b0 (snd s) i
--     Iso.rightInv (PIso C) (f , p) = ΣPathP (funExt (λ x → funExt λ y → p x a0 y b0 ∙ p a0 x b0 y) , {!funE!})
--     Iso.leftInv (PIso C) = {!!}
...
-- --   the-coh-pplr-gen i j = hcomp (λ r' → λ {(i = i0) → sq1 r' j
...
-- --   the-coh-pplr-f f i j r =
-- --     hfill (λ r → λ {(i = i0) → push (false , (λ x → push (x , λ i → f i x) (~ j))) r
-- --                     ; (i = i1) → doubleCompPath-filler (sym (push (false , λ i → inl (i , CasesBool {A = λ x → A x x} true (f true true) (f false false) i))))
-- --                                   (λ i → inl (false , push (false , CasesBool true (f true false) (f false false)) i))
-- --                                   refl r (~ j)
-- --                     ; (j = i0) → compPath-filler (λ i → inl (false , inr (CasesBoolη (λ x → f x false) i)))
-- --                                    (push (false , λ y → inr (λ x → f x y))) r (~ i)
-- --                     ; (j = i1) → push (false , (λ x → inl (x , CasesBoolη (λ x → f x x) (~ i) x))) r})
-- --       (inS (inl (false , push (false , CasesBoolη (λ x → f x false) (~ i)) (~ j))))
-- --       r

-- --   the-coh-pplr-t : (f : TotΠ (λ i → TotΠ (A i))) (i j r : I)
-- --     → join-gen Bool (λ j → join-gen Bool (λ i → A i j))
-- --   the-coh-pplr-t f i j r =
-- --     hfill (λ r → λ {(i = i0) → push (true , (λ x → push (x , (λ i → f i x)) (~ j))) r
-- --                    ; (i = i1) → doubleCompPath-filler (sym (push (true , λ i → inl (i , CasesBool {A = λ x → A x x} true (f true true) (f false false) i))))
-- --                                   (λ i → inl (true , push (true , CasesBool true (f true true) (f false true)) i))
-- --                                   refl r (~ j)
-- --                    ; (j = i0) → compPath-filler
-- --                                   (λ k → inl (true , inr (CasesBoolη (λ x → f x true) k)))
-- --                                   (push (true , λ j₁ → inr (λ x → f x j₁))) r (~ i)
-- --                    ; (j = i1) → push (true , λ x → inl (x , CasesBoolη (λ x → f x x) (~ i) x)) r})
-- --           (inS (inl (true , push (true , CasesBoolη (λ x → f x true) (~ i)) (~ j))))
-- --           r
...
-- -- module _ {A : Bool → Bool → Type} (f : (x y : Bool) → A x y) where
-- --   fun₃-coh-false-Sq : Square ((sym (push (false , (λ x → inl (true , f true x))))
-- --                              ∙ λ j → inl (false , push (true , CasesBool {A = λ x → A x false} true (f true false) (f false false)) j)))
-- --                              (push (true , λ x → inl (x , CasesBool {A = λ x → A x x} true (f true true) (f false false) x)))
-- --                              (sym (push (true , λ x → inl (true , f true x))))
-- --                              (((λ i → inl (false , push (false , CasesBool {A = λ x → A x false} true (f true false) (f false false)) (~ i)))
-- --                              ∙ push (false , λ x → inl (x , CasesBool {A = λ x → A x x} true (f true true) (f false false) x))))

-- --   fun₃-coh-false-Sq i j = fun₃-coh-false₃ {A = A} (f false false) (f true) i j i1

-- --   fun₃-coh-false-Sq2 : Square {A = join-gen Bool (λ j → join-gen Bool (λ i → A i j))}
-- --                             ((sym (push (false , (λ x → inl (true , f true x))))
-- --                              ∙ λ j → inl (false , push (true , CasesBool {A = λ x → A x false} true (f true false) (f false false)) j)))
-- --                              (push (true , λ x → inl (x , CasesBool {A = λ x → A x x} true (f true true) (f false false) x)))
-- --                              (sym (push (true , λ x → inl (true , f true x))))
-- --                              (((λ i → inl (false , push (false , CasesBool {A = λ x → A x false} true (f true false) (f false false)) (~ i)))
-- --                              ∙ push (false , λ x → inl (x , CasesBool {A = λ x → A x x} true (f true true) (f false false) x))))
-- --   fun₃-coh-false-Sq2 i j =
-- --     hcomp (λ r → λ {(i = i0) → (sym (push (false , (λ x → inl (true , f true x))))
-- --                                ∙ λ j → inl (false , push (true , CasesBoolη (λ x → f x false) (~ r)) j)) j
-- --                    ; (i = i1) → push (true , λ x → inl (x , CasesBoolη (λ x → f x x) (~ r) x )) j
-- --                    ; (j = i0) → push (true , λ x → inl (true , f true x)) (~ i)
-- --                    ; (j = i1) → (((λ i → inl (false , push (false , CasesBoolη (λ x → f x false) (~ r)) (~ i)))
-- --                                 ∙ push (false , λ x → inl (x , CasesBoolη (λ x → f x x) (~ r) x)))) i})
-- --       (hcomp (λ r → λ {(i = i0) → compPath-filler (sym (push (false , (λ x → push (true ,  λ y → f y x) (~ r)))))
-- --                                                     ((λ j₁ → inl (false , push (true , (λ x → f x false)) (j₁ ∨ ~ r)))) i1 j
-- --                    ; (i = i1) → push (true , λ x → push (x , (λ y → f y x)) (~ r)) j
-- --                    ; (j = i0) → push (true , λ x → push (true , (λ y → f y x)) (~ r)) (~ i)
-- --                    ; (j = i1) → {!compPath-filler !}})
-- --               {!!}) -- (push (true , (λ x → inr (λ y → f y x))) (~ i ∨ j)))

-- --   fun₃-coh-false-f : fun₃-coh-false-Sq ≡ fun₃-coh-false-Sq2
-- --   fun₃-coh-false-f = {!!}
...
-- --                    ; (i = i1) → {!!}
-- --                    ; (k = i0) → {!!}
-- --                    ; (k = i1) → {!fun₃-coh-false₃ (f false false) (f true) i j i1!}})
-- --           {!!}
