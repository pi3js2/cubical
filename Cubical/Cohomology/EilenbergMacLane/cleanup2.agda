...
module _ (I J : RP∞' ℓ) (A : fst I → fst J → Type ℓ) where
  ...
  ΠR-extend₃→JoinR-pushᵣ i' j (inl (inl (f , p , q))) = sym (pushR (j , f (notRP∞' i')) p q)
  ...
  ΠR-extend₃→JoinR-push : (i' : fst I) (j : fst J) (a : ΠJoinRBack i' j) (i : fst I)
    → ΠΣ→ΠJoinR I J A i' (ΠJoinRBack→L i' j a .snd .fst) (ΠJoinRBack→L i' j a .snd .snd) i
     ≡ ΠR-base₂→JoinR I J A i (ΠJoinRBack→ΠR-base i' j a)
...
module _ (J : RP∞' ℓ) (A : Bool → fst J → Type ℓ) where
  ...
  leftMapBool i (inl (i' , a , b)) = ΠΣ→ΠJoinR (RP∞'∙ _) J A i' a b i
  leftMapBool i (inr x) = ΠR-base₂→JoinR (RP∞'∙ _) J A i x
  leftMapBool false (push (false , j , a) k) =
    ΠR-extend₃→JoinR-pushₗ (RP∞'∙ _) J A false j a k
  leftMapBool false (push (true , j , a) k) =
    ΠR-extend₃→JoinR-pushᵣ (RP∞'∙ _) J A true j a k
  leftMapBool true (push (false , j , a) k) =
    ΠR-extend₃→JoinR-pushᵣ (RP∞'∙ _) J A false j a k
  leftMapBool true (push (true , j , a) k) =
    ΠR-extend₃→JoinR-pushₗ (RP∞'∙ _) J A true j a k

  leftMapBool≡ : (i : Bool)
    (x : ΠJoinR₃.ΠR-extend₃ (RP∞'∙ ℓ) J A)
    → leftMapBool i x ≡ ΠR-extend₃→JoinR (RP∞'∙ _) J A i x
  leftMapBool≡ i (inl x) = refl
  leftMapBool≡ i (inr x) = refl
  leftMapBool≡ false (push (false , j , a) i₁) k =
    lUnit (ΠR-extend₃→JoinR-pushₗ (RP∞'∙ _) J A false j a) k i₁
  leftMapBool≡ true (push (false , j , a) i₁) k =
    lUnit (ΠR-extend₃→JoinR-pushᵣ (RP∞'∙ _) J A false j a) k i₁
  leftMapBool≡ false (push (true , j , a) i₁) k =
    lUnit (ΠR-extend₃→JoinR-pushᵣ (RP∞'∙ _) J A true j a) k i₁
  leftMapBool≡ true (push (true , j , a) i₁) k =
    lUnit (ΠR-extend₃→JoinR-pushₗ (RP∞'∙ _) J A true j a) k i₁

leftMapsAgree : (I J : RP∞' ℓ) (A : fst I → fst J → Type ℓ)
...
leftMapsAgree {ℓ = ℓ} I J A i (push (i' , a) k) l = help I i' A i a l k
  ...
  module _ (J : RP∞' ℓ) (A : Bool → fst J → Type ℓ) where
    ...
    fill₁-refl : (f : fst J) (g : (i : Bool) (j : fst J) → A i j)
               → cong (leftMapBool J A false) ((λ j → inl (true , (f , g true f) , g false)) ∙ push (true , f , inr g)) ≡ refl
    fill₁-refl f g i j = leftMapBool J A false (compPath-filler' refl (push (true , f , inr g)) (~ i) j)

    fill₂-refl : (f : fst J) (g : (i : Bool) (j : fst J) → A i j)
               → cong (leftMapBool J A true) ((λ j → inl (true , (f , g true f) , g false)) ∙ push (true , f , inr g)) ≡ pushR (f , g true f) (g true) refl
    fill₂-refl f g i j = leftMapBool J A true (compPath-filler' refl (push (true , f , inr g)) (~ i) j)

    abstract
      fill₁ :  (f : fst J) (g : (i : Bool) (j : fst J) → A i j) (a : A true f)  (q : g true f ≡ a)
               → cong (leftMapBool J A false) ((λ j → inl (true , (f , q (~ j)) , g false)) ∙ push (true , f , inr g)) ≡ refl
      fill₁ f g = J> (fill₁-refl f g)

      fill₂ :  (f : fst J) (g : (i : Bool) (j : fst J) → A i j) (a : A true f)  (q : g true f ≡ a)
               → cong (leftMapBool J A true) ((λ j → inl (true , (f , q (~ j)) , g false)) ∙ push (true , f , inr g)) ≡ pushR (f , a) (g true) q
      fill₂ f g = J> (fill₂-refl f g)

      fill₁-refl≡ : (f : fst J) (g : (i : Bool) (j : fst J) → A i j) → fill₁ f g (g true f) refl ≡ fill₁-refl f g
      fill₁-refl≡ f g =
    ...
    fill-inl : (x : fst J) (p : (i : Bool) (j : fst J) → A i j)
            ...
            → Cube (λ j k → pushR (x , f false)  (p false) (funExt⁻ q false) (~ k))
                    (fill₁ x p (f true) (funExt⁻ q true))
                    (λ i k → leftMapBool J A false (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push (RP∞'∙ ℓ) J A true (push ((inl x , f) , p , (funExt⁻ q)) i) k))
    ...
    fill-inl x p = J> ((λ i j k → leftMapBool J A false
                        (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push-fillerₗ (RP∞'∙ _) J A true x _ p (λ _ → refl) i k (~ j)))
                     ▷ sym (fill₁-refl≡ x p))
    ...
    fill-inr : (x : Bool ≃ fst J) (p : (i : Bool) (j : fst J) → A i j)
            ...
            → Cube (λ j k → pushR (fst x false , f false)  (p false) (funExt⁻ q false) (~ k))
                    ...
                    (λ i k → leftMapBool J A false (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push (RP∞'∙ ℓ) J A true (push ((inr x , f) , p , (funExt⁻ q)) i) k))
                    (λ i k → pushR (fst x false , f false)  (p false) (funExt⁻ q false) (~ k ∨ i))
                    (λ _ _ → inrR (p false))
                    λ i j → pushR (fst x false , f false) (p false) (funExt⁻ q false) i
    fill-inr x p = J> ((λ i j k → leftMapBool J A false (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push-fillerᵣ (RP∞'∙ _) J A true x _ p (λ _ → refl) i k (~ j)))
    ...
    fill₂-inl : (x : fst J) (p : (i : Bool) (j : fst J) → A i j)
            ...
            → Cube (λ j k → inlR (x , f true))
                    (fill₂ x p (f true) (funExt⁻ q true))
                    (λ i k → leftMapBool J A true (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push (RP∞'∙ ℓ) J A true (push ((inl x , f) , p , (funExt⁻ q)) i) k))
                    (λ i k → pushR (x , f true)  (p true) (funExt⁻ q true) (k ∧ i))
                    (λ i j → inlR (x , f true))
                    λ i j → pushR (x , f true) (p true) (funExt⁻ q true) i
    fill₂-inl x p =
      J> ((λ i j k → leftMapBool J A true
           (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push-fillerₗ (RP∞'∙ _) J A true x
             _ p (λ _ → refl) i k (~ j)))
      ▷ sym (fill₂-refl≡ x p))
    ...
    fill₂-inr : (x : Bool ≃ fst J) (p : (i : Bool) (j : fst J) → A i j)
            ...
            → Cube (λ j k → inlR (fst x true , f true))
                    (fill₂ (fst x true) p (f true) (funExt⁻ q true))
                    (λ i k → leftMapBool J A true (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push (RP∞'∙ ℓ) J A true (push ((inr x , f) , p , (funExt⁻ q)) i) k))
      ...
      asd i j k =
        hcomp (λ r → λ {(i = i0) → inlR (fst x true , p true (fst x true))
                   ...
                   ; (j = i0) → leftMapBool J A true
                                  (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push-fillerᵣ (RP∞'∙ ℓ) J A true x
    ...
    mainₗ : (x : _)
      → cong (leftMapBool J A false) (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push (RP∞'∙ ℓ) J A true x)
    ...
    mainᵣ : (x : _)
      → cong (leftMapBool J A true) (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push (RP∞'∙ ℓ) J A true x)
    ...
    main : (k : _) (x : _)
      → cong (leftMapBool J A k) (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃-push (RP∞'∙ ℓ) J A true x)
      ≡ cong (ΠR-extend₂→JoinR (RP∞'∙ ℓ) J A k) (push (true , x))
    main false x = mainₗ x ∙ lUnit _
    main  true x = mainᵣ x ∙ lUnit _

  h1 : (J : RP∞' ℓ) (A : Bool → fst J → Type ℓ) (k : _) → leftMapBool J A k ≡ ΠR-extend₃→JoinR (RP∞'∙ _) J A k
  h1 J A k = funExt (leftMapBool≡ J A k)
  help : (I : RP∞' ℓ) (i' : fst I) (A : fst I → fst J → Type ℓ) (i : fst I) (a : _)
    → cong (ΠR-extend₃→JoinR I J A i) (cong (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃ I J A) (push (i' , a)))
     ≡ cong (ΠR-extend₂→JoinR I J A i) (push (i' , a))
  help = JRP∞' λ A k x → (λ j → cong (h1 J A k (~ j))
      (cong (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃ (RP∞'∙ ℓ) J A) (push (true , x))))
      ∙ main J A k x
...
UnordJoin²₂→UnordJoin²-alt I J A (inr x) = inr (ΠJoinR₃.ΠR-extend₂→ΠR-extend₃ I J A x)
UnordJoin²₂→UnordJoin²-alt I J A (push a i) =
  ((λ t → inl (fst a , leftMapsAgree I J A (fst a) (snd a) (~ t)))
  ∙ push (fst a , ΠJoinR₃.ΠR-extend₂→ΠR-extend₃ I J A (snd a))) i
...
--     → PathP (λ k → Targ (RP∞'∙ ℓ) (RP∞'∙ ℓ) A
--                       (push (true , true , inl (inr (idEquiv Bool , refl , g , f , p))) k))
--              (inler A (g true) f)
...
--       → (x : ΠJoinR₃.ΠJoinRBack (RP∞'∙ ℓ) J A true j) → PathP (λ k → Targ (RP∞'∙ ℓ) J A (push (true , j , x) k))
--            (ΠR-extend→Inl (RP∞'∙ ℓ) true J (fst (fst (ΠJoinR₃.ΠJoinRBack→leftPush (RP∞'∙ ℓ)  J A true j x))) A
--              (snd (fst (ΠJoinR₃.ΠJoinRBack→leftPush (RP∞'∙ ℓ) J A true j x))) (snd (ΠJoinR₃.ΠJoinRBack→leftPush (RP∞'∙ ℓ) J A true j x)))
--            (ΠR-extend→Inr (RP∞'∙ ℓ) J A (ΠJoinR₃.ΠJoinRBack→ΠR-base (RP∞'∙ ℓ) J A true j x))
--     cohr' = JRP∞' cohr-b

--     cohr : (I : RP∞' ℓ) (i : fst I) (J : RP∞' ℓ) (j : fst J) (A : fst I → fst J → Type ℓ)
--       → (x : ΠJoinR₃.ΠJoinRBack I J A i j) → PathP (λ k → Targ I J A (push (i , j , x) k))
--            (ΠR-extend→Inl I i J (fst (fst (ΠJoinR₃.ΠJoinRBack→leftPush I J A i j x))) A
--              (snd (fst (ΠJoinR₃.ΠJoinRBack→leftPush I J A i j x))) (snd (ΠJoinR₃.ΠJoinRBack→leftPush I J A i j x)))
...
--       lem = JRP∞' (JRP∞' λ A k a → help' A k a ▷ sym (cohr-id A k a))
...
-- MEGA-inst↓ : (Targ : (I J : RP∞' ℓ) (A : fst I → fst J → Type ℓ) → ΠJoinR₃.ΠR-extend₃ I J A → Type ℓ)
...
--      λ A x a j → GFUN A (ΣJoinR→JoinR² (RP∞'∙ ℓ) (RP∞'∙ ℓ) A (a , leftMapBool (RP∞'∙ ℓ) A a (push x j)))
...
--                  (λ I J A x i → ΣJoinR→JoinR² I J A (i , ΠR-extend₃→JoinR I J A i x))
--                  (λ A x a j → GFUN A (ΣJoinR→JoinR² (RP∞'∙ ℓ) (RP∞'∙ ℓ) A (a , leftMapBool (RP∞'∙ ℓ) A a (push x j))))
--                  (λ t A x y i → GFUN A (ΣJoinR→JoinR² (RP∞'∙ ℓ) (RP∞'∙ ℓ) A (y , leftMapBool≡ (RP∞'∙ ℓ) A y (push x i) (~ t))))
--                  𝕄inst· I J A
