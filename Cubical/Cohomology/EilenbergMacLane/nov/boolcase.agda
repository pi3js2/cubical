...
leftFun*-fullBool J A false (push (true , y) k) = {!!}
leftFun*-fullBool J A true (push (false , a) k) = {!!}
leftFun*-fullBool J A true (push (true , y) k) = {!!}


leftMapBool≡' : ∀ {ℓ} (J : RP∞' ℓ) (A : Bool → fst J → Type ℓ)
  → (i : Bool) (x : ΠR-extend* (RP∞'· ℓ) J A)
  → leftFun*-full (RP∞'· _) J A i x ≡ {!leftFun* J A x!} -- leftMapBool J A i x ≡  {!!}
leftMapBool≡' = {!!}
