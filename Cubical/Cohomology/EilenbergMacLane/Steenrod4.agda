...
module Cubical.Cohomology.EilenbergMacLane.Steenrod4 where
...
isInHab2 : (X : Type) → is2Type ℓ-zero X → ∥ X ∥₁
isInHab2 X p = {!p .snd (λ _ → ∥ X ∥₁) .fst!}

isPr-is2TypeBool : isContr (is2Type ℓ-zero Bool)
fst isPr-is2TypeBool = Bool-2Type _
snd isPr-is2TypeBool (f , p) = {!!}
  where
  notConst : f false ≡ f true → ⊥
  notConst r = true≢false (isContr→isProp isContrBool _ _)
    where
    hel : f false ≡ true → (x : Bool) → x ≡ f false 
    hel ha = p _ .fst true (sym ha) (sym r)

    hel2 : f false ≡ false → (x : Bool) → x ≡ f false 
    hel2 ha = p _ .fst false (sym ha) refl
  ...
  help' : (x y : Bool) → f x ≡ y → f x ≡ not x
  help' false false p = {!p!}
  help' true false p = p
  help' false true p = p
  help' true true p = {!!}
...
isPr-is2Type* : (X : RP∞) → isProp (is2Type ℓ-zero (fst X))
isPr-is2Type* X (f1 , p) (f2 , q) = Σ≡Prop {!λ t  !} {!!}
...
{-
  PushTop≃Main : (a : fst I) →
      Pushout (2nd.fat→ₗ a) (2nd.fat→ᵣ a) ≃
      Pushout (1st.fat→ₗ a) (1st.fat→ᵣ a)
  PushTop≃Main i = isoToEquiv (pushoutIso _ _ _ _ is1 is2 is3
    (funExt (uncurry λ p a → {!(1st.fat→ₗ i ∘ fst is1) (p , a)!}))
    {!fst is2 (p , a i (2nd.eval p i) , a (2nd.notI i))!})
    where
    is1 : 2nd.fat ≃ 1st.fat
    is1 = isoToEquiv
      (invIso (compIso
        (invIso (Σ-cong-iso-fst (invIso (TotAIso I J {A = A}))))
        (compIso
          Σ-assoc-Iso
          (Σ-cong-iso-snd λ f
            → compIso (invIso Σ-assoc-Iso)
              (compIso
                ...
                (compIso
                  (compIso Σ-assoc-Iso
                    (Σ-cong-iso-snd λ g → compIso (Σ-cong-iso-snd (λ f → funExtIso))
                      (isContr→Iso (isContrSingl _) (isContrUnit))))
                  rUnit×Iso))))))

    is2 : 2nd.left-push↑ₗ i ≃ 1st.left-push↑ₗ i
    is2 = isoToEquiv
      (invIso (compIso
        (invIso (Σ-cong-iso-fst (invIso (TotAIso I J {A = A}))))
        (compIso Σ-assoc-Iso
          (Σ-cong-iso-snd λ f
            → compIso (compIso (invIso Σ-assoc-Iso)
                (compIso (Σ-cong-iso-fst Σ-swap-Iso) Σ-assoc-Iso))
                 (compIso
                   (Σ-cong-iso-snd
                     (λ r → invIso (helpIso33 i f r)))
                   Σ-swap-Iso)))))

    is3 : 2nd.left-push↑ᵣ i ≃ 1st.left-push↑ᵣ i
...
module _ {ℓ : Level} (I J : RP∞) (A : fst I → fst J → Type ℓ) (I2 : ∀ {ℓ} → is2Type ℓ (fst I)) where
  private
    module 1st = 2-elter (fst I) (fst J) A I2
    module 2nd = 2-elter' I (fst J) A I2
  ...
  Isor = pushoutIso _ _ _ _ (isoToEquiv (invIso is1)) (isoToEquiv (invIso (TotAIso I J {A = A}))) (idEquiv _)
    ...
    is1 = compIso (invIso (Σ-cong-iso-fst (invIso (TotAIso I J))))
      ...
      h : (f : fst J ⊎ (fst I ≃ fst J)) (r : (i : fst I) (j : fst J) → A i j)
        → isContr (Σ[ g ∈ ((i : fst I) → A i (fst (2-eltFun {I} {J}) f i)) ]
                      (Path ((i : fst I) → _) (λ x → r x (fst (2-eltFun {I} {J}) f x)) λ x → g x))
  ...
  Iso.fun (helpIso33 i f r) a = (1st.elimI {B = λ x → A x (evalG f x)} i a (r _)) , sym (1st.elimIβ {B = λ x → A x (evalG f x)} i a (r _) .snd)
  Iso.inv (helpIso33 i f r) (a , p) = a i
  Iso.rightInv (helpIso33 i f r) (a , p) =
    ΣPathP ((funExt (1st.elimI {B = λ x → 1st.elimI {B = λ x → A x (evalG f x)} i (a i) (r _) x ≡ a x} i (1st.elimIβ i (a i) (r (evalG f (fst I2 i))) .fst)
                                 (1st.elimIβ i (a i) (r (evalG f (fst I2 i))) .snd ∙∙ refl ∙∙ p)))
      , flipSquare (doubleCompPath-filler (1st.elimIβ i (a i) (r (evalG f (fst I2 i))) .snd) refl p
        ▷ ((sym (1st.elimIβ {B = λ x → 1st.elimI {B = λ x → A x (evalG f x)} i (a i) (r _) x ≡ a x}
          i (1st.elimIβ i (a i) (r (evalG f (fst I2 i))) .fst)
             (1st.elimIβ i (a i) (r (evalG f (fst I2 i))) .snd ∙∙ refl ∙∙ p) .snd)))))
...
-}
  ...
  PushTop≃ : 2nd.PushTop ≃ 1st.PushTop
  PushTop≃ = Σ-cong-equiv-snd {!!} -- PushTop≃Main

  ΠR-extendTransf : Iso 2nd.ΠR-extend 1st.ΠR-extend
  ΠR-extendTransf = pushoutIso _ _ _ _ {!!} (idEquiv _) ΠR-base≃ (funExt (uncurry {!!})) (funExt (uncurry {!λ i → ?!}))


PushTop≃Main : {ℓ : Level} (I : RP∞) (i : fst I) (J : RP∞)
  (A : fst I → fst J → Type ℓ)
  (I2 : ∀ {ℓ} → is2Type ℓ (fst I))
  → Σ[ F ∈ Pushout (2-elter'.fat→ₗ I (fst J) A I2 i)
                    (2-elter'.fat→ᵣ I (fst J) A I2 i)
          ≃ Pushout (2-elter.fat→ₗ (fst I) (fst J) A I2 i)
                    (2-elter.fat→ᵣ (fst I) (fst J) A I2 i) ]
      ((y : Pushout (2-elter'.fat→ₗ I (fst J) A I2 i)
                    (2-elter'.fat→ᵣ I (fst J) A I2 i))
      → ((2-elter'.PushTop→left-push I (fst J) A I2 (i , y)
       ≡ 2-elter.PushTop→left-push (fst I) (fst J) A I2 (i , fst F y)))
       × (Isor-alt I J A I2 (2-elter'.PushTop→ΠR-base I (fst J) A I2 (i , y))
       ≡ 2-elter.PushTop→ΠR-base (fst I) (fst J) A I2 (i , fst F y)))
PushTop≃Main = {!JRP∞ ? ?!}
